...
template <typename T>
concept Simple =
    std::default_initializable<T> && std::is_trivially_destructible_v<T>;
...
class RingBuffer {
 public:
  ...
  void Store(std::int64_t i,
             T&& val) noexcept requires std::is_nothrow_move_assignable_v<T> {
    buffer_[i & mask_] = std::move(val);
  }

  T Load(std::int64_t i)
      const noexcept requires std::is_nothrow_move_constructible_v<T> {
